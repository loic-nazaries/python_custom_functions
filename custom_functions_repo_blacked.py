...
def check_normality_assumption_residuals(
    ...
    shapiro_wilk = pg.normality(
        data=dataframe, method="shapiro", alpha=(1 - confidence_interval)
    )
    shapiro_wilk.rename(index={0: "Shapiro-Wilk"}, inplace=True)

    normality = pg.normality(
        data=dataframe, method="normaltest", alpha=(1 - confidence_interval)
    )
    normality.rename(index={0: "Normality"}, inplace=True)

    jarque_bera = pg.normality(
        data=dataframe, method="jarque_bera", alpha=(1 - confidence_interval)
    )
...
def draw_kdeplot_subplots(
    ...
    for index, feature in enumerate(feature_list):
        ...
        axis.set_xlabel(
            ...
            xlabel=(" ".join([word.capitalize() for word in feature.split("_")])),
            fontsize=18,
        )
...
def draw_kde_plots(dataframe, columns, label):
    ...
    fig.suptitle(
        f"Distribution of {label.capitalize()} Features (with Skewness)", fontsize=20
    )
...
def draw_correlation_heatmap(
    ...
    correlation_heatmap.set_yticklabels(
        labels=correlation_heatmap.get_yticklabels(), rotation="horizontal", size=12
    )
...
def run_exploratory_data_visualisation(
    ...
    plt.suptitle(
        t=f"Effects of {group_variable} on detection parameters", fontsize=28, y=0.92
    )
    save_figure(
...
def calculate_cross_validation_prediction_scores(
    ...
    for score in score_list:
        score_name = score.__name__.replace("_", " ").replace("score", "").capitalize()
